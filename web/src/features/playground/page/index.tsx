...
export default function PlaygroundPage() {
  ...
  const getMessageSearchPageLabel = useCallback(
    (_pageId: string, pageIndex: number) =>
      windowIds.length > 1 ? t("playground.page.windowLabel", { index: String(pageIndex + 1) }) : null,
    [windowIds.length],
  );
  ...
  const executionStatus = (() => {
    const rawStatus = getExecutionStatus();
    if (!rawStatus) {
      if (globalIsExecutingAll) {
        return t("playground.header.executingAllWindows", {
          count: String(windowIds.length),
        });
      }
      return null;
    }
    // Parse hook's English status patterns and translate them
    const allMatch = rawStatus.match(/^Executing all (\d+) windows$/);
    if (allMatch) {
      return t("playground.header.executingAllWindows", {
        count: allMatch[1],
      });
    }
    const someMatch = rawStatus.match(/^Executing (\d+) of (\d+) windows$/);
    if (someMatch) {
      return t("playground.header.executingSomeWindows", {
        executing: someMatch[1],
        total: someMatch[2],
      });
    }
    return rawStatus;
            ...
            <div className="flex flex-nowrap items-center gap-2">
              ...
              <div className="text-muted-foreground hidden items-center gap-2 text-sm md:flex">
                <span className="whitespace-nowrap">
                  {t("playground.header.windowCount", { count: String(windowIds.length) })}
                </span>
                {executionStatus && (
                  <>
              ...
              <Button
                ...
                title={
                  !hasAnyModelConfigured
                    ? t("playground.header.runAllTooltipNoModel")
                    : t("playground.header.runAllTooltip")
                }
              >
                ...
                <span className="hidden items-center gap-1 lg:inline-flex">
                  <span>{t("playground.header.runAll")}</span>
