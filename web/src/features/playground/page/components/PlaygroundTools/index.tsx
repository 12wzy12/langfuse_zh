...
export const PlaygroundToolsPopover = () => {
  ...
  return (
    <Command className="flex flex-col">
      <CommandInput
        placeholder={t("playground.configDropdowns.searchTools")}
        className="h-8 border-none py-1 pr-1 pl-6 focus:ring-0 focus:ring-offset-0"
      />
      <CommandList className="max-h-[300px] overflow-y-auto">
        <CommandEmpty>{t("playground.configDropdowns.noToolsFound")}</CommandEmpty>
        <CommandGroup>
          {savedTools.map((tool) => (
            <CommandItem
              key={tool.id}
      ...
      <div className="mt-auto p-1">
        ...
        >
          <Button variant="outline" size="default" className="w-full">
            ...
            {t("playground.configDropdowns.createNewTool")}
          </Button>
        </CreateOrEditLLMToolDialog>
      </div>
    </Command>
    ...
    <ScrollArea className="[&>[data-radix-scroll-area-viewport]]:max-h-[min(45vh,18rem)]">
      {tools.length === 0 ? (
        <div className="flex h-16 flex-col items-center justify-center p-4 text-center">
          <p className="text-muted-foreground text-xs">{t("playground.configDropdowns.noToolsAttached")}</p>
        </div>
      ) : (
            ...
            >
              <div className="bg-background hover:bg-accent/50 relative cursor-pointer rounded-md border p-2 pr-10 transition-colors duration-200">
                <Button
                  ...
                  aria-label={t("playground.configDropdowns.removeTool", {name: tool.name})}
                  onClick={(e) => {
                    e.stopPropagation();
                    e.preventDefault();
