...
export const StructuredOutputSchemaPopover = () => {
  ...
  return (
    <Command className="flex flex-col">
      <CommandInput
        placeholder={t("playground.configDropdowns.searchSchemas")}
        className="h-8 border-none py-1 pr-1 pl-6 focus:ring-0 focus:ring-offset-0"
      />
      <CommandList className="max-h-[300px] overflow-y-auto">
        <CommandEmpty>{t("playground.configDropdowns.noSchemasFound")}</CommandEmpty>
        <CommandGroup>
      ...
      <div className="mt-auto p-1">
        ...
        >
          <Button variant="outline" size="default" className="w-full">
            ...
            {t("playground.configDropdowns.createNewSchema")}
          </Button>
        </CreateOrEditLLMSchemaDialog>
      </div>
    </Command>
    ...
    <ScrollArea className="max-h-[min(45vh,18rem)]">
      {!structuredOutputSchema ? (
        <div className="flex h-16 flex-col items-center justify-center p-4 text-center">
          <p className="text-muted-foreground text-xs">{t("playground.configDropdowns.noSchemaProvided")}</p>
        </div>
      ) : (
          ...
          >
            <div className="bg-background hover:bg-accent/50 relative cursor-pointer rounded-md border p-2 pr-10 transition-colors duration-200">
              <Button
                ...
                aria-label={t("playground.configDropdowns.removeSchema", {name: structuredOutputSchema.name})}
                onClick={(e) => {
                  e.stopPropagation();
                  e.preventDefault();
                  handleRemoveSchema();
                ...
                <div className="min-w-0">
                  ...
                  {!isSchemaSaved(structuredOutputSchema) ? (
                    <span className="bg-muted text-muted-foreground mt-1 inline-flex rounded px-1 py-0.5 text-xs">
                      {t("playground.configDropdowns.unsaved")}
                    </span>
                  ) : null}
                </div>
                <p
