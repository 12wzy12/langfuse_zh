  ...
  const handleInputChange = useCallback(
    (jsonString: string) => {
      try {
        ...
        setError(t("playground.messagePlaceholders.invalidJson"));
      }
    },
    ...
    <div className="p-1">
      <div className="mb-1 flex flex-row items-center">
        <span className="flex flex-1 flex-row space-x-2 text-xs">
          ...
          <p
            ...
            title={name}
          >
            {name ? name : t("playground.messagePlaceholders.unnamedPlaceholder")}
          </p>
        </span>
        <Button
          variant="ghost"
          size="icon"
          title={t("playground.messagePlaceholders.deletePlaceholder")}
          disabled={isUsed}
      ...
      {hasConflict && (
        <p className="mt-1 text-xs text-red-500">
          {t("playground.messagePlaceholders.conflictError")}
        </p>
      )}
