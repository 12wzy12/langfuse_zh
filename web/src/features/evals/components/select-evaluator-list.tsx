    ...
    <>
      <div className="mb-4 flex max-h-full min-h-0 flex-col gap-5">
        <div className="shrink-0 space-y-2">
          <h2 className="text-base font-semibold">{t("evals.selectEvaluator.createFromScratch")}</h2>
          <div className="flex flex-wrap gap-3">
            {isCodeEvalEnabled ? (
              ...
              >
                ...
                <span className="flex flex-col gap-1">
                  <span className="font-medium">{t("evals.selectEvaluator.codeEvaluator")}</span>
                  <span className="text-muted-foreground text-sm font-normal">
                    {t("evals.selectEvaluator.codeEvalDesc")}
                  </span>
                </span>
              </Button>
            ) : null}
            ...
            >
              ...
              <span className="flex flex-col gap-1">
                <span className="font-medium">{t("evals.selectEvaluator.llmJudge")}</span>
                <span className="text-muted-foreground text-sm font-normal">
                    {t("evals.selectEvaluator.llmJudgeDesc")}
                </span>
              </span>
            </Button>
          </div>
        ...
        <div className="flex max-h-full min-h-0 flex-col gap-2">
          <h2 className="shrink-0 text-base font-semibold">{t("evals.selectEvaluator.useExisting")}</h2>
            ...
            <div className="flex min-h-0 flex-col overflow-hidden">
              {templates.isLoading ? (
                ...
                <div className="text-destructive py-8 text-center">
                  {t("evals.selectEvaluator.error")}: {templates.error.message}
                </div>
              ) : templates.data?.templates.length === 0 ? (
                <div className="text-muted-foreground py-8 text-center">
                  {t("evals.selectEvaluator.noEvaluators")}
                </div>
              ) : (
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle>{t("evals.selectEvaluator.createNewEval")}</DialogTitle>
          </DialogHeader>
          <EvalTemplateForm
            ...
            onFormSuccess={(newTemplate) => {
              ...
              showSuccessToast({
                title: t("evals.selectEvaluator.createSuccess"),
                description: t("evals.selectEvaluator.createSuccessDesc"),
              });
            }}
          />
        </DialogContent>
