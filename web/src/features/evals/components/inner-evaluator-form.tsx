  ...
  function onSubmit(values: z.infer<typeof evalConfigFormSchema>) {
    ...
    if (
      ...
      form.setError("timeScope", {
        type: "manual",
        message: t("evals.form.runOnChanged"),
      });
      return;
    }
    ...
    if (validatedFilter.success === false) {
      form.setError("filter", {
        type: "manual",
        message: t("evals.form.fillAllFilters"),
      });
      return;
    }
    ...
    if (
      ...
      form.setError("target", {
        type: "manual",
        message: t("evals.form.invalidTarget"),
      });
      return;
    }
    ...
    if (
      ...
      form.setError("mapping", {
        type: "manual",
        message: t("evals.form.traceEvalNoLongerSupported"),
      });
      return;
    }
    ...
    >
      <div className={cn(shouldShowCodeEvalTestPanel && "xl:col-span-2")}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("evals.form.generatedScoreName")}</FormLabel>
              <FormControl>
                <Input {...field} />
              </FormControl>
              <FormMessage />
        ...
        <Card className="flex max-w-full flex-col gap-2 overflow-y-auto p-4">
          {hasInvalidTraceFilters && (
            <Alert variant="destructive">
              ...
              <AlertTitle>{t("evals.form.unsupportedFilterTitle")}</AlertTitle>
              <AlertDescription>
                {t("evals.form.unsupportedFilterDesc")}
              </AlertDescription>
            </Alert>
          )}
            ...
            {!props.hideTargetSelection &&
              ...
              props.evalCapabilities.allowLegacy && (
                <div className="flex flex-col gap-2">
                  <FormLabel className="text-sm">{t("evals.form.experimentMethod")}</FormLabel>
                  <Tabs
                  ...
                  >
                    <TabsList className="grid w-fit max-w-fit grid-flow-col gap-4">
                      ...
                      >
                        ...
                        {t("evals.form.lowLevelSdk")}
                        <Badge
                          variant="secondary"
                          size="sm"
                          className="border-border border font-normal"
                        >
                          {t("evals.form.legacy2")}
                        </Badge>
                      </TabsTrigger>
                    </TabsList>
                  </Tabs>
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="items-top flex space-x-2">
                            ...
                            <div className="grid gap-1.5 leading-none">
                              ...
                              >
                                {t("evals.form.new", { target: getTargetDisplayName(form.watch("target")) })}
                              </label>
                            </div>
                          </div>
                          <div className="items-top flex space-x-2">
                            ...
                            <div className="flex items-center gap-1.5 leading-none">
                              <label
                                ...
                                className="text-sm leading-none font-medium peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                              >
                                {t("evals.form.existing", { target: getTargetDisplayName(form.watch("target")) })}
                              </label>
                              {field.value.includes("EXISTING") &&
                                !props.disabled &&
                                (props.mode === "edit" ? (
                                  <Tooltip>
                                    ...
                                    <TooltipContent className="max-w-[300px] p-2">
                                      <span className="leading-4">
                                        {t("evals.form.existingTooltip", { target: getTargetDisplayName(form.watch("target")) })}
                                      </span>
                                    </TooltipContent>
                                  </Tooltip>
                                ) : (
                          ...
                          <div className="space-y-0.5">
                            <FormLabel>
                              {isEventTarget(target)
                                ? t("evals.form.runOnLive")
                                : t("evals.form.runOnNewExp")}
                            </FormLabel>
                            <FormDescription>
                              {t("evals.form.autoEvalDesc", { target: getTargetDisplayName(target) })}
                            </FormDescription>
                          </div>
                          <FormControl>
                            <Switch
              ...
              <>
                <FormField
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <FormItem>
                        <FormLabel>{t("evals.form.filter")}</FormLabel>
                        <FormControl>
                          <div className="max-w-[500px]">
                            {props.disabled && !hasFilters ? (
                              <p className="text-muted-foreground text-xs">
                                {t("evals.form.allTargetEvaluated", { target: getTargetDisplayName(target) })}
                              </p>
                            ) : (
                              <InlineFilterBuilder
                                key={target}
                        ...
                        {!props.disabled && !hasFilters && (
                          <div className="align-center flex max-w-[500px] gap-1">
                            ...
                            <AlertDescription className="text-dark-yellow">
                              {t("evals.form.noFiltersSet", { target: getTargetDisplayName(target) })}
                            </AlertDescription>
                          </div>
                        )}
                        <FormMessage />
                  ...
                  <>
                    ...
                    {isLegacyEvalTarget(form.watch("target")) && (
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>{t("evals.form.delay")}</FormLabel>
                            <FormControl>
  ...
  const formFooter = props.renderFooter ? (
    ...
    <div className="flex w-full flex-col items-end gap-4">
      {!props.disabled ? (
        ...
        >
          {props.mode === "edit" ? t("evals.form.update") : t("evals.form.execute")}
        </Button>
      ) : null}
      {formError ? (
        <p className="text-red w-full text-center">
          <span className="font-bold">{t("evals.form.error")}:</span> {formError}
        </p>
      ) : null}
    </div>
  );
