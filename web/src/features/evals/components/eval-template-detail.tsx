  ...
  return (
    <Page
      headerProps={{
        ...
        breadcrumb: [
          {
            name: t("evals.breadcrumb.evaluatorLibrary"),
    ...
    >
      {allTemplates.isLoading || !allTemplates.data || !template.data ? (
        <div className="p-3">{t("evals.loading")}</div>
      ) : isEditing ? (
        ...
        <div className="grid flex-1 grid-cols-[1fr_auto] overflow-hidden contain-layout">
          ...
          <SidePanel mobileTitle={t("evals.changeHistory")} id="change-history">
            <SidePanelHeader>
              <SidePanelTitle className="text-base font-semibold">
                {t("evals.changeHistory")}
              </SidePanelTitle>
            </SidePanelHeader>
            <SidePanelContent>
  ...
  return (
    ...
    >
      <SelectTrigger className="w-[180px]">
        <SelectValue placeholder={t("evals.version")} />
      </SelectTrigger>
      <SelectContent>
        <SelectGroup>
...
export function UpdateTemplate({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-sm font-medium">{t("evals.editMode")}</span>
      <Switch
        checked={isEditing}
