...
export default function ConfigureRetention() {
  ...
  return (
    <div>
      <Header title={t("settings.dataRetention.title")} />
      <Card className="mb-4 p-3">
        ...
        form.getValues().retention !== project?.retentionDays ? (
          <p className="text-primary mb-4 text-sm">
            {t("settings.dataRetention.changingRetention", {
              old: project?.retentionDays ? `${project.retentionDays}` : t("settings.dataRetention.indefinite"),
              new: Number(form.watch("retention")) === 0 ? t("settings.dataRetention.indefinite") : `${Number(form.watch("retention"))}`,
            })}
          </p>
        ) : !Boolean(project?.retentionDays) ? (
          <p className="text-primary mb-4 text-sm">
            {t("settings.dataRetention.retainsIndefinitely")}
          </p>
        ) : (
          <p className="text-primary mb-4 text-sm">
            {t("settings.dataRetention.currentRetention", {
