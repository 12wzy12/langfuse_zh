...
function ProjectsGroup({ onNavigate }: { onNavigate: () => void }) {
  ...
  return (
    <>
      <CommandSeparator />
      <CommandGroup heading={t("commandMenu.projects")}>
...
function DashboardsGroup({ onNavigate }: { onNavigate: () => void }) {
  ...
  return (
    <>
      <CommandSeparator />
      <CommandGroup heading={t("commandMenu.dashboards")}>
        {dashboards.map((dashboard) => (
          <CommandItem
            key={dashboard.id}
            value={t("commandMenu.dashboardItem", { name: dashboard.name })}
            keywords={[
              "dashboard",
            ...
            onSelect={() => {
              ...
              capture("cmd_k_menu:navigated", {
                type: "dashboard",
                title: t("commandMenu.dashboardItem", {
                  name: dashboard.name,
                }),
                url: url,
              });
...
function ProjectSettingsGroup({ onNavigate }: { onNavigate: () => void }) {
  ...
  const projectSettingsItems = settingsPages
    ...
    .map((page) => ({
      title: t("commandMenu.projectSettingsItem", { title: page.title }),
  ...
  return (
    <>
      <CommandSeparator />
      <CommandGroup heading={t("commandMenu.projectSettings")}>
...
function OrganizationSettingsGroup({ onNavigate }: { onNavigate: () => void }) {
  ...
  const orgSettingsItems = orgSettingsPages
    .filter((page) => page.show !== false && !("href" in page))
    .map((page) => ({
      title: t("commandMenu.organizationSettingsItem", { title: page.title }),
  ...
  return (
    <>
      <CommandSeparator />
      <CommandGroup heading={t("commandMenu.organizationSettings")}>
...
function AccountSettingsGroup({ onNavigate }: { onNavigate: () => void }) {
  ...
  const accountSettingsItems = accountSettingsPages.map((page) => ({
    title: t("commandMenu.accountSettingsItem", { title: page.title }),
  ...
  return (
    <>
      <CommandSeparator />
      <CommandGroup heading={t("commandMenu.accountSettings")}>
