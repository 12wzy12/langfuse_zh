  ...
  return (
    <div className="relative">
      <div className="mb-1 flex items-center justify-between">
        <FormLabel className="text-sm">{t("scores.annotationForm.scoreComment")}</FormLabel>
        <div className="relative">
          {savedComment && (
      ...
      {hasChanges && (
        <div className="mt-2 flex justify-end gap-1">
          <PopoverClose asChild>
            ...
            >
              {t("scores.annotationForm.discardChanges")}
            </Button>
          </PopoverClose>
          <PopoverClose asChild>
            <Button
            ...
            >
              {t("scores.annotationForm.saveChanges")}
            </Button>
          </PopoverClose>
        </div>
      )}
...
function AnnotateHeader({
  ...
  return (
    <Header
      title={t("scores.annotationForm.headerTitle")}
      help={{
        description,
      ...
      actionButtons={[
        <div className="flex items-center justify-end" key="saving-spinner">
          ...
          <span className="text-muted-foreground text-xs">
            {showSaving ? t("scores.annotationForm.savingScoreData") : t("scores.annotationForm.scoreDataSaved")}
          </span>
        </div>,
        actionButtons,
      ]}
...
function InnerAnnotationForm<Target extends ScoreTarget>({
  ...
  const rollbackDeleteError = (
    ...
    if (isTextDataType(field.dataType)) {
      form.setError(`scoreData.${index}.stringValue`, {
        type: "server",
        message: t("scores.annotationForm.failedToDeleteScore"),
      });
    } else {
  ...
  const rollbackUpdateError = (
    ...
    if (isTextDataType(controlledFields[index]?.dataType)) {
      form.setError(`scoreData.${index}.stringValue`, {
        type: "server",
        message: t("scores.annotationForm.failedToUpdateScore"),
      });
    } else {
      form.setError(`scoreData.${index}.value`, {
        type: "server",
  ...
  const rollbackCreateError = (
    ...
    if (isTextDataType(controlledFields[index]?.dataType)) {
      form.setError(`scoreData.${index}.stringValue`, {
        type: "server",
        message: t("scores.annotationForm.failedToCreateScore"),
      });
    } else {
      form.setError(`scoreData.${index}.value`, {
        type: "server",
  ...
  const rollbackCommentError = (
    ...
    form.setError(`scoreData.${index}.comment`, {
      type: "server",
      message: t("scores.annotationForm.failedToUpdateComment"),
    });
  };
        ...
        {allowManualSelection ? (
          <div className="grid grid-flow-col items-center">
            <MultiSelectKeyValues
              placeholder={t("scores.annotationForm.valuePlaceholder")}
              align="end"
              items={t("scores.annotationForm.emptyScores")}
              ...
              controlButtons={
                ...
                >
                  {t("scores.annotationForm.manageScoreConfigs")}
                </DropdownMenuItem>
              }
            />
          </div>
                          ...
                          <Popover>
                            <PopoverTrigger asChild>
                              <Button
                                ...
                                type="button"
                                size="xs"
                                title={t("scores.annotationForm.addOrViewComment")}
                        ...
                        <div className="grid grid-cols-[11fr_1fr] items-center py-1">
                                ...
                                <FormItem>
                                  <FormControl>
                                    <Textarea
                                      ...
                                      className="text-xs"
                                      disabled={isInputDisabled(config)}
                                      placeholder={t("scores.annotationForm.enterFreeFormText")}
                                ...
                                <FormItem>
                                  <FormControl>
                                    <Combobox
                                      ...
                                      placeholder={t("scores.annotationForm.selectCategory")}
                                      searchPlaceholder={t("scores.annotationForm.searchCategories")}
                                      emptyText={t("scores.annotationForm.noCategoryFound")}
                                    />
                                  </FormControl>
                                  <FormMessage className="text-xs" />
                                </FormItem>
                          ...
                          {config.isArchived ? (
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button
                                  ...
                                  type="button"
                                  className="px-0 pl-1"
                                  title={t("scores.annotationForm.deleteArchivedScore")}
                              ...
                              <PopoverContent>
                                <h2 className="text-md mb-3 font-semibold">
                                  {t("scores.annotationForm.scoreIsArchived")}
                                </h2>
                                <p className="mb-3 text-sm">
                                  {t("scores.annotationForm.deleteIrreversibly")}
                                </p>
                                <div className="flex justify-end space-x-4">
                                  ...
                                  >
                                    {t("scores.annotationForm.delete")}
                                  </Button>
                                </div>
                              </PopoverContent>
                            </Popover>
                            ...
                            <Button
                              ...
                              type="button"
                              className="px-0 pl-1"
                              title={t("scores.annotationForm.deleteFromTrace")}
                              disabled={
                                isScoreUnsaved(score.id) ||
                                updateMutation.isPending
                              }
