...
export function ApiKeyList(props: { entityId: string; scope: ApiKeyScope }) {
  ...
  if (!hasAccess) {
    return (
      <div>
        <Header title={t("settings.apiKeys.title")} />
        <Alert>
          <AlertTitle>{t("settings.apiKeys.accessDenied")}</AlertTitle>
          <AlertDescription>
            {t("settings.apiKeys.accessDeniedDescription", { scope })}
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <Header
        ...
        help={{
          description: t("settings.apiKeys.learnMore", { scope }),
          href:
            scope === "project"
      ...
      <CodeView
        content={envCode}
        title={t("settings.apiKeys.envTitle")}
        copiedToClipboardMessage={t("settings.apiKeys.copiedToClipboardMessage")}
      />
          ...
          <TableHeader>
            <TableRow>
              <TableHead className="text-primary hidden md:table-cell">
                ...
                {t("settings.apiKeys.publicKey")}
                {t("settings.apiKeys.secretKey")}
...
function DeleteApiKeyButton(props: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mb-5">{t("settings.apiKeys.deleteTitle")}</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          ...
          >
            {t("settings.apiKeys.permanentlyDelete")}
          </Button>
...
function ApiKeyNote({
  ...
  return (
    ...
    >
      {note || t("settings.apiKeys.notePlaceholderClick")}
    </div>
  );
}
