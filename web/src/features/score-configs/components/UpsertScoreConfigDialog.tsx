...
export function UpsertScoreConfigDialog({
  ...
  const createScoreConfig = api.scoreConfigs.create.useMutation({
    ...
    onError: (error) =>
      setFormError(error.message ?? t("scores.upsertConfig.createError")),
  });

  const updateScoreConfig = api.scoreConfigs.update.useMutation({
    onSuccess: () => utils.scoreConfigs.invalidate(),
    onError: (error) =>
      setFormError(error.message ?? t("scores.upsertConfig.updateError")),
  });
      ...
      >
        <DialogTrigger asChild>
          <Button variant="secondary" loading={createScoreConfig.isPending}>
            ...
            {id ? t("scores.upsertConfig.updateButton") : t("scores.upsertConfig.addButton")}
          </Button>
        </DialogTrigger>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {id ? t("scores.upsertConfig.titleUpdate") : t("scores.upsertConfig.titleCreate")}
            </DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
              <DialogBody>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("scores.upsertConfig.name")}</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          type="text"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("scores.upsertConfig.dataType")}</FormLabel>
                      <Select
                        disabled={!!id}
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder={t("scores.upsertConfig.dataTypePlaceholder")} />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                ...
                {isNumericDataType(form.getValues("dataType")) ? (
                  <>
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("scores.upsertConfig.minimum")}</FormLabel>
                          <FormControl>
                            <Input
                              {...field}
                              value={field.value ?? ""}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("scores.upsertConfig.maximum")}</FormLabel>
                          <FormControl>
                            <Input
                              {...field}
                              value={field.value ?? ""}
                        ...
                        <>
                          {fields.length > 0 && (
                            <div className="mb-2 grid grid-cols-[1fr_3fr] items-center gap-2 text-left sm:grid-cols-[1fr_7fr]">
                              <FormLabel className="grid grid-flow-col">
                                {t("scores.upsertConfig.value")}
                                <DocPopup
                                  description={t("scores.upsertConfig.valueDocPopup", {
                                    type: isCategoricalDataType(form.getValues("dataType"))
                                      ? t("scores.upsertConfig.category")
                                      : t("scores.upsertConfig.boolean"),
                                  })}
                                />
                              </FormLabel>
                              <FormLabel>{t("scores.upsertConfig.label")}</FormLabel>
                            </div>
                          )}
                          ...
                          {isCategoricalDataType(
                            ...
                            <div className="grid-cols-auto grid">
                              ...
                              >
                                {t("scores.upsertConfig.addCategory")}
                              </Button>
                            </div>
                          )}
                        </>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <>
                      <FormItem>
                        <FormLabel>{t("scores.upsertConfig.description")}</FormLabel>
                        <FormControl>
                          <Textarea
                            {...field}
                            placeholder={t("scores.upsertConfig.descriptionPlaceholder")}
                ...
                <div className="flex w-full flex-col items-end gap-4">
                  {formError ? (
                    <p className="text-red w-full text-center">
                      <span className="font-bold">{t("scores.upsertConfig.error")}:</span> {formError}
                    </p>
                  ) : null}
                  <Button
                    type="submit"
