...
export const PromptDetail = ({
  ...
  const handleExperimentSuccess = async (data?: {
    ...
    showSuccessToast({
      title: t("prompts.detail.experimentTriggeredTitle"),
      description: t("prompts.detail.experimentTriggeredDescription"),
      link: {
  ...
  if (!promptHistory.data || !prompt) {
    return <div className="p-3">{t("prompts.detail.loading")}...</div>;
  }
  ...
  return (
    <Page
      headerProps={{
        title: prompt.name,
        titleTooltip: t("prompts.detail.titleTooltip"),
        itemType: "PROMPT",
        help: {
          description: t("prompts.detail.helpDescription"),
          ...
          >
            <TabsBarList className="max-w-full min-w-0 justify-start overflow-x-auto">
              <TabsBarTrigger value="prompt">
                {t("prompts.detail.promptTab")}
              </TabsBarTrigger>
              <TabsBarTrigger value="config">
                {t("prompts.detail.configTab")}
              </TabsBarTrigger>
              <TabsBarTrigger value="linked-generations">
                {t("prompts.detail.linkedGenerationsTab")}
              </TabsBarTrigger>
              <TabsBarTrigger value="use-prompt">
                {t("prompts.detail.usePromptTab")}
              </TabsBarTrigger>
            </TabsBarList>
            <TabsBarContent
            ...
            >
              <div className="flex h-full min-h-0 w-full flex-col gap-2 overflow-y-auto pb-4">
                {pythonCode && (
                  <CodeView
                    content={pythonCode}
                    title={t("prompts.detail.python")}
                  />
                )}
                {jsCode && (
                  <CodeView content={jsCode} title={t("prompts.detail.jsTs")} />
                )}
                <p className="text-muted-foreground pl-1 text-xs">
                  {t("prompts.detail.docsText")}{" "}
                  <a
                    href="https://langfuse.com/docs/prompts"
                    className="underline"
                    target="_blank"
