...
export const AnnotationQueueItemPage: React.FC<{
  ...
  if (!relevantItem && !(itemId && seenItemIds.includes(itemId))) {
    return <div>{t("annotationQueues.itemPage.noMoreItems")}</div>;
  }
          ...
          <span className="text-muted-foreground max-w-96 text-sm text-wrap">
            <span
              dangerouslySetInnerHTML={{
                __html: t("annotationQueues.itemPage.deletedItemTitle"),
              }}
            />{" "}
            <span
              dangerouslySetInnerHTML={{
                __html: t("annotationQueues.itemPage.deletedItemDesc"),
              }}
            />
          </span>
        </Card>
        ...
        <div className="flex w-full min-w-[265px] justify-end gap-2">
          ...
          {!!relevantItem &&
            (relevantItem.status === AnnotationQueueStatus.PENDING ? (
              ...
              >
                {t("annotationQueues.itemPage.markCompleted")}
              </Button>
            ) : (
              <div className="text-dark-gree border-dark-green bg-light-green inline-flex h-9 w-full items-center justify-center rounded-md border px-8 text-sm font-medium">
                {t("annotationQueues.itemPage.completed")}
              </div>
            ))}
        </div>
      </div>
