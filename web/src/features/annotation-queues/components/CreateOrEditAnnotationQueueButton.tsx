...
export const CreateOrEditAnnotationQueueButton = ({
  ...
  useUniqueNameValidation({
    ...
    form,
    errorMessage: t("annotationQueues.createEditQueue.queueNameExists"),
  });
  ...
  const onSubmit = async (data: CreateQueueWithAssignments) => {
    try {
      ...
      showErrorToast(
        t("annotationQueues.createEditQueue.operationFailed"),
        t("annotationQueues.createEditQueue.operationFailedDesc"),
      );
    }
  };
  ...
  const handleOnValueChange = (values: Record<string, string>[]) => {
    ...
    if (values.length === 0) {
      form.setError("scoreConfigIds", {
        type: "manual",
        message: t("annotationQueues.createEditQueue.atLeastOneScoreConfig"),
      });
    } else {
      ...
      <DialogTrigger asChild>
        ...
        >
          <span className="ml-1 text-sm font-normal">
            {queueId
              ? t("annotationQueues.createEditQueue.edit")
              : t("annotationQueues.createEditQueue.newQueue")}
          </span>
        </ActionButton>
      </DialogTrigger>
      {configsData.data && (
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {queueId
                ? t("annotationQueues.createEditQueue.dialogTitleEdit")
                : t("annotationQueues.createEditQueue.dialogTitleNew")}
            </DialogTitle>
            <DialogDescription>
              {queueId
                ? t("annotationQueues.createEditQueue.dialogDescEdit")
                : t("annotationQueues.createEditQueue.dialogDescNew")}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
              <DialogBody>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("annotationQueues.createEditQueue.name")}</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          type="text"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("annotationQueues.createEditQueue.description")}</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          placeholder={t("annotationQueues.createEditQueue.descriptionPlaceholder")}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("annotationQueues.createEditQueue.scoreConfigs")}</FormLabel>
                      <FormDescription>
                        {t("annotationQueues.createEditQueue.scoreConfigsDesc")}
                      </FormDescription>
                      <FormControl>
                        <MultiSelectKeyValues
                          ...
                          controlButtons={
                            ...
                            >
                              {t("annotationQueues.createEditQueue.manageScoreConfigs")}
                            </DropdownMenuItem>
                          }
                        />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("annotationQueues.createEditQueue.advancedSettings")}</FormLabel>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium">
                                  {t("annotationQueues.createEditQueue.userAssignment")}
                                </span>
                              </div>
                            </Button>
                          </CollapsibleTrigger>
              ...
              <DialogFooter>
                ...
                >
                  ...
                  editQueueMutation.isPending ||
                  createQueueAssignmentsMutation.isPending
                    ? t("annotationQueues.createEditQueue.processing")
                    : queueId
                      ? t("annotationQueues.createEditQueue.saveQueue")
                      : t("annotationQueues.createEditQueue.createQueue")}
                </Button>
              </DialogFooter>
            </form>
          </Form>
