...
function CloneDashboardButton({
  ...
  const mutCloneDashboard = api.dashboard.cloneDashboard.useMutation({
    onSuccess: () => {
      ...
      showSuccessToast({
        title: t("dashboards.list.clonedTitle"),
        description: t("dashboards.list.clonedDescription"),
      });
    },
    onError: (e) => {
      showErrorToast(t("dashboards.list.cloneFailed"), e.message);
    },
  });

  const handleCloneDashboard = () => {
    if (!projectId) {
      console.error(t("dashboards.list.projectIdMissing"));
      return;
    }
...
export function DashboardTable() {
  ...
  const dashboardColumns = [
    ...
    columnHelper.accessor("description", {
      header: t("dashboards.list.descriptionColumn"),
      id: "description",
      size: 300,
      cell: (row) => {
        return row.getValue();
    ...
    columnHelper.display({
      id: "ownerTag",
      header: t("dashboards.list.owner"),
      size: 80,
      cell: (row) => {
        return row.row.original.owner === "LANGFUSE" ? (
          ...
          <span className="flex gap-1 px-2 py-0.5 text-xs">
            <UserIcon className="h-3 w-3" /> {t("dashboards.list.project")}
          </span>
        );
      },
    }),
    columnHelper.accessor("createdAt", {
      header: t("dashboards.list.createdAt"),
      id: "createdAt",
      enableSorting: true,
      size: 150,
      cell: (row) => {
    ...
    columnHelper.accessor("updatedAt", {
      header: t("dashboards.list.updatedAt"),
      id: "updatedAt",
      enableSorting: true,
      size: 150,
      cell: (row) => {
    ...
    columnHelper.display({
      id: "actions",
      header: t("dashboards.list.actions"),
      size: 70,
      cell: (row) => {
                ...
                {owner === "PROJECT" && (
                  <DropdownMenuItem asChild>
                    <DeleteDashboardButton
                      ...
                      projectId={projectId}
                      isTableAction
                      customDeletePrompt={t("dashboards.list.deletePrompt")}
                    />
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
