...
export function TracePage({
  ...
  useEffect(() => {
    if (isBetaEnabled && eventsData.cutoffObservationsAfterMaxCount) {
      showErrorToast(
        t("tracing.detail.truncatedTitle"),
        t("tracing.detail.truncatedDescription"),
        "WARNING",
      );
    }
  ...
  if (!isBetaEnabled && tracesQuery.error?.data?.code === "UNAUTHORIZED")
    return <ErrorPage message={t("tracing.detail.accessDenied")} />;

  if (!isBetaEnabled && tracesQuery.error?.data?.code === "NOT_FOUND")
    return (
      <ErrorPage
        title={t("tracing.detail.notFoundTitle")}
        message={t("tracing.detail.notFoundDescription")}
        additionalButton={{
          label: t("tracing.detail.retry"),
  ...
  if (isBetaEnabled && !eventsData.isLoading && !eventsData.data)
    return (
      <ErrorPage
        title={t("tracing.detail.notFoundTitle")}
        message={t("tracing.detail.notFoundDescriptionBeta")}
        additionalButton={{
          label: t("tracing.detail.retry"),
          onClick: () => window.location.reload(),
        }}
  ...
  if (!trace.data) return <div className="p-3">{t("common.loading")}...</div>;
  ...
  const leadingControl = showPublicIndicators ? (
    session.status === "authenticated" ? (
      <Button
        ...
        size="sm"
        variant="outline"
        title={t("tracing.detail.backToLangfuse")}
        className="px-3"
      >
      ...
      <Button
        ...
        size="sm"
        variant="default"
        title={t("tracing.detail.signInToLangfuse")}
        className="px-3"
      >
        <Link href={`/auth/sign-in?targetPath=${encodedTargetPath}`}>
          {t("tracing.detail.signIn")}
        </Link>
      </Button>
    )
  ) : undefined;
  ...
  return (
    <Page
      headerProps={{
        ...
        breadcrumb: [
          {
            name: t("tracing.detail.breadcrumb"),
