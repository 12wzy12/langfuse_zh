                    ...
                    >
                      <Badge className="max-w-[260px]">
                        <span className="truncate">
                          {t("sessions.detail.userId", { userId })}
                        </span>
                        <ExternalLinkIcon className="ml-1 h-3 w-3" />
                      </Badge>
                    </Link>
            ...
            {remainingUsers.length > USERS_PER_PAGE_IN_POPOVER && (
              <div className="flex items-center justify-between border-t p-2 pt-4">
                <Button
                  ...
                  disabled={page === 0}
                >
                  {t("sessions.detail.previous")}
                </Button>
                <span className="text-muted-foreground text-sm">
                  {t("sessions.detail.pageCount", {
                    page: page + 1,
                    total: Math.ceil(
                      remainingUsers.length / USERS_PER_PAGE_IN_POPOVER,
                    ),
...
export const SessionPage: React.FC<{
  ...
  if (session.error?.data?.code === "UNAUTHORIZED")
    return <ErrorPage message={t("sessions.detail.accessDenied")} />;

  if (session.error?.data?.code === "NOT_FOUND")
    return (
      <ErrorPage
        title={t("sessions.detail.notFoundTitle")}
        message={t("sessions.detail.notFoundDescription")}
        additionalButton={{
          label: t("sessions.detail.retry"),
    ...
    <SessionDetailStoreProvider store={sessionDetailStore}>
      <Page
        headerProps={{
          ...
          breadcrumb: [
            {
              name: t("sessions.detail.breadcrumb"),
          ...
          actionButtonsRight: (
            <>
              ...
              <Button
                ...
                title={t("sessions.detail.downloadJson")}
              >
              ...
              <div className="flex items-center">
                ...
                <span className="text-muted-foreground text-xs">
                  {t("sessions.detail.showCorrections")}
                </span>
              </div>
            </>
          ),
        ...
        <div className="flex h-full flex-col overflow-auto">
          <div className="bg-background sticky top-0 z-40 flex flex-wrap gap-2 border-b p-4">
            ...
            <Badge variant="outline">
              {t("sessions.detail.totalTraces", {
                count: session.data?.traces.length ?? 0,
              })}
            </Badge>
            {session.data && (
              <Badge variant="outline">
                {t("sessions.detail.totalCost", {
...
export const SessionEventsPage: React.FC<{
  ...
  const positionInTraceColumn: ColumnDefinition = React.useMemo(
    () => ({
      name: t("sessions.detail.positionInTrace"),
      id: "positionInTrace",
  ...
  const timeFiltersForOptions = React.useMemo(() => {
    ...
    return allFilters.filter(
      (f) =>
        (f.column === t("tracing.events.columns.startTime") ||
          f.column === "startTime") &&
        f.type === "datetime",
    );
  }, [filtersQuery, sessionEventsFilterConfig.columnDefinitions, t]);
  ...
  const visibleFilterState = React.useMemo(
    () =>
      queryFilter.filterState.filter(
        (filter) =>
          filter.column !== t("sessions.detail.filterOutSessionId") &&
          filter.column !== "sessionId" &&
          filter.column !==
            t("sessions.detail.filterOutHasParentObservation") &&
