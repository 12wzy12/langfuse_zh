...
export default function DashboardDetail() {
  ...
  const updateDashboardDefinition =
    api.dashboard.updateDashboardDefinition.useMutation({
      onSuccess: () => {
        showSuccessToast({
          title: t("dashboards.detail.updatedTitle"),
          description: t("dashboards.detail.autoSavedDescription"),
          duration: 2000,
        });
      ...
      onError: (error) => {
        showErrorToast(t("dashboards.detail.updateFailed"), error.message);
      },
    });
  ...
  const updateDashboardFilters =
    api.dashboard.updateDashboardFilters.useMutation({
      onSuccess: () => {
        showSuccessToast({
          title: t("dashboards.detail.filtersSavedTitle"),
          description: t("dashboards.detail.filtersSavedDescription"),
          duration: 2000,
        });
      ...
      onError: (error) => {
        showErrorToast(t("dashboards.detail.saveFiltersFailed"), error.message);
      },
    });
  ...
  const filterColumns: ColumnDefinition[] = [
    {
      name: t("dashboards.detail.filters.environment"),
      id: "environment",
      type: "stringOptions",
    ...
    {
      name: t("dashboards.detail.filters.traceName"),
      id: "traceName",
      type: "stringOptions",
      options: nameOptions,
      internal: "internalValue",
    ...
    {
      name: t("dashboards.detail.filters.observationName"),
      id: "observationName",
      type: "string",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.scoreName"),
      id: "scoreName",
      type: "string",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.tags"),
      id: "tags",
      type: "arrayOptions",
      options: tagsOptions,
      internal: "internalValue",
    ...
    {
      name: t("dashboards.detail.filters.user"),
      id: "user",
      type: "string",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.session"),
      id: "session",
      type: "string",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.metadata"),
      id: "metadata",
      type: "stringObject",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.release"),
      id: "release",
      type: "string",
      internal: "internalValue",
    },
    {
      name: t("dashboards.detail.filters.version"),
      id: "version",
      type: "string",
      internal: "internalValue",
    },
  ...
  const mutateCloneDashboard = api.dashboard.cloneDashboard.useMutation({
    ...
    onError: (e) => {
      showErrorToast(t("dashboards.detail.cloneFailed"), e.message);
    },
  });
  ...
  return (
    ...
    >
      <Page
        ...
        headerProps={{
          title:
            (dashboard.data?.name || t("dashboards.detail.fallbackTitle")) +
            (dashboard.data?.owner === "LANGFUSE"
              ? ` ${t("dashboards.detail.maintainedSuffix")}`
              : ""),
          breadcrumb: [
            {
              name: t("dashboards.detail.breadcrumb"),
              href: `/project/${projectId}/dashboards`,
            },
          ],
          help: {
            description:
              dashboard.data?.description || t("dashboards.detail.noDescription"),
          },
          actionButtonsRight: (
            <>
              {hasCUDAccess && hasUnsavedFilterChanges && (
                ...
                >
                  {updateDashboardFilters.isPending
                    ? t("dashboards.detail.saving")
                    : t("dashboards.detail.saveFilters")}
                </Button>
              )}
              {hasCUDAccess && (
                <Button onClick={handleAddWidget}>
                  <PlusIcon size={16} className="mr-1 h-4 w-4" />
                  {t("dashboards.detail.addWidget")}
                </Button>
              )}
              {hasCloneAccess && (
                <Button
