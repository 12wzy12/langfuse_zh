...
export default function PosthogIntegrationSettings() {
  ...
  return (
    ...
    >
      <p className="text-primary mb-4 text-sm">{t("settings.posthog.wrapperDescription")}</p>
      {!hasAccess && (
        <p className="text-sm">
          {t("settings.posthog.noAccess")}
        </p>
      )}
      {hasAccess && (
        <>
          <Header title={t("settings.posthog.configuration")} />
          <Card className="p-3">
      ...
      {state.data?.enabled && (
        <>
          <Header title={t("settings.posthog.status")} className="mt-8" />
          <p className="text-primary text-sm">
            {t("settings.posthog.dataSyncedUntil")}{" "}
    ...
    <Form {...posthogForm}>
      <form className="space-y-3" onSubmit={posthogForm.handleSubmit(onSubmit)}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("settings.posthog.hostname")}</FormLabel>
              <FormControl>
                <Input {...field} />
              </FormControl>
              <FormDescription>
                {t("settings.posthog.hostnameDescription")}
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("settings.posthog.apiKey")}</FormLabel>
              <FormControl>
                <PasswordInput {...field} />
              </FormControl>
              <FormMessage />
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="flex items-center gap-1.5 pt-2">
                  {t("settings.posthog.exportSource")}
                  <Tooltip>
                    <TooltipTrigger>
                    ...
                    >
                      ...
                      <div className="border-t pt-2">
                        ...
                        >
                          {t("settings.posthog.forFurtherInfo")}
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue placeholder={t("settings.posthog.selectDataToExport")} />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
                    {EXPORT_SOURCE_OPTIONS.map((option) => (
                ...
                <FormDescription>
                  {t("settings.posthog.exportSourceDescription")}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("settings.posthog.enabled")}</FormLabel>
              <FormControl>
                <Switch
      ...
      <div className="mt-8 flex gap-2">
        ...
        <Button
          ...
          onClick={() => {
            if (confirm(t("settings.posthog.resetConfirm")))
              mutDelete.mutate({ projectId });
          }}
        >
          {t("settings.posthog.reset")}
        </Button>
      </div>
    </Form>
  );
